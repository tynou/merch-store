...
func TestGetUserInfo_Success(t *testing.T) {
	...
	mockStore := mocks.NewMockQuerier(t)

	mockStore.On("GetUserBalance", ctx, userID).Return(int32(1000), nil)

	mockStore.On("GetUserInventory", ctx, userID).Return([]db.GetUserInventoryRow{
		{Type: "t-shirt", Quantity: 2},
		{Type: "book", Quantity: 1},
	}, nil)

	mockStore.On("GetReceivedTransfers", ctx, userID).Return([]db.GetReceivedTransfersRow{
		{FromUser: "user1", Amount: 100},
	}, nil)

	mockStore.On("GetSentTransfers", ctx, userID).Return([]db.GetSentTransfersRow{
		{ToUser: "user2", Amount: 50},
		{ToUser: "user3", Amount: 67},
	}, nil)

	service := NewInfoService(mockStore, mockStore, mockStore)
...
func TestGetUserInfo_GetUserBalance_Failure(t *testing.T) {
	...
	mockStore.On("GetUserBalance", ctx, userID).Return(int32(0), apperr.ErrNotFound)
...
func TestGetUserInfo_GetUserInventory_Failure(t *testing.T) {
	...
	mockStore.On("GetUserInventory", ctx, userID).Return([]db.GetUserInventoryRow{}, errors.New("some error"))
